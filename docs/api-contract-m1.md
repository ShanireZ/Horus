...
| `exam_ended` | **考试结束**(考试派发 2026-07-03) | `examId`。`POST /api/exams/{id}/end` 时推给该考试所有在线 Agent;Agent 离线错过的,重连 `hello` 时按考试状态**补发**。Agent 收到:留 5s 排空缓冲续传,然后停采、弃会话回待命 |
| `session_revoked` | **全场远程登出**(同上) | `examId`。`POST /api/exams/{id}/logout` 吊销该考试全部 OIDC 会话后推送,随后**强断**连接(吊销的会话不能续用);Agent 立即停采回待命,重连 401 须重新登录 |
| `ping` / `pong` | 保活 | `ts` |
...
| `ocr_results` | 视觉 LLM 识图结果（L2·**视觉 LLM 已取代 OCR**·表名 `ocr_results` 沿用） | `image_id, text, hits, confidence` |
| `logo_hits` | Logo 匹配（L3·已并入 L2 视觉级·表名沿用） | `image_id, label, score, bbox` |
| `image_embeddings` | CLIP 向量（**普通 BLOB 表 + C# 暴力余弦检索·未用 sqlite-vec**；`vec_images` 虚表已删，死代码移除） | `image_id, embedding BLOB` |
...
- `images.image_id → ocr_results / logo_hits / image_embeddings`（一图多分析）。
...
5. （M2 起）将需要视觉判定的图按 §5/architecture 收口送**视觉 LLM 识图**（已取代 OCR），回填 `ocr_results`（表名沿用）。

> L2 视觉 LLM 识图（取代 OCR）、L3 Logo（并入 L2）、CLIP 按图搜图（已落地·C# 暴力余弦·未用 sqlite-vec）、归档作业属 M2/M3，本契约预留字段，M1 不实现。
>
> **M3 增补(管理面,已实现)**：
...
> - **考试派发批(2026-07-03)**:
>   - `GET /oidc/active-exam` — **公开**(Agent 待命轮询,无凭证):`{ active:false }` 或 `{ active:true, examId, name }`。派发判据 = status='active' 中最近创建的一场。
>   - `GET /oidc/session` — **会话探针**(头 `X-Horus-Session`,sessionId 即凭证):`{ valid:false }` 或 `{ valid:true, examId, seatId, examStatus }`。Agent 每 60s 兜底探测「被远程登出/考试结束」。
>   - `POST /api/exams/{examId}/end` — 响应增加 `notified`(收到 `exam_ended` 推送的在线连接数)。
>   - `POST /api/exams/{examId}/logout` — **全场远程登出**(admin 门内):吊销该考试全部 OIDC 采集会话 + 推 `session_revoked` + 强断连接,返回 `{ ok, examId, revoked, notified }`。
>   - `POST /oidc/exchange` — 请求体**不再收 examId/seatId**(收 `{code, codeVerifier, redirectUri, nonce, agentEcdhPub, agentId, machineId}`);响应增加 `examId`(服务端派发)与 `seatId`(= OIDC username,不安全/空回退 sub)。无活跃考试 → 401 `no_active_exam`(先于 token 交换判定,不白耗一次性授权码)。
