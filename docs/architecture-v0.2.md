...
- **PSK 驻留学员机**：Agent 跑在学员机、PSK 在本地配置。持 PSK 者可对事件正确签名。
  ...
  - **⏳ 仍属 M3**：服务器仍**不重算 canonical、不验哈希链连续性**（`server_risk` 只从 payload 元数据独立推导,不复算 `hashSelf`）。持 PSK 者仍可伪造 payload 内容本身（如把真实 AI 站 URL 写成判题站）——此类靠截图 / L2 OCR 与人工裁决兜底。**完整 canonical 复算 + 哈希链复验属 M3**。缓解：每 Agent 独立 PSK（泄露隔离到单座位）、密钥用机器保护而非明文。
...
- **M2/M3 待办**：**归档/清理定时任务尚未实现**（schema 承诺 30 天转 archive，当前无执行者 → 长期常驻会无界增长，属 §13/§15 M3）；心跳在哈希链上，归档 DELETE 会断链（M3 哈希链复验时需分段容忍）。
