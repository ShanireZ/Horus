...
- **PSK 驻留学员机**：Agent 跑在学员机、PSK 在本地配置。持 PSK 者可对事件正确签名。
  ...
  - **✅ M3 已实现「完整 canonical 复算 + 哈希链复验」**（闭合本条大半）：
    ...
    - **离线审计** `IntegrityAudit` + `GET /api/exams/{examId}/integrity`:按 (agent,seq) 复验 ①锚点自洽(落库后 payload/字段被改 → 报 hashMismatch)②链连续(hash_prev 链前驱,删除/插入/重排 → 报 continuityBreak)。迁移前缺 machine_id 的旧行归 `unverifiable`,**不误报篡改**。
...
- **✅ 已做（M2）读写分离**：`Db` 从「单连接 + 全局写锁」改为 **单写连接（写锁串行）+ 独立只读连接（WAL 并发读）**；看板 6 个只读 GET 端点走只读连接,与采集写路径**互不阻塞**（监考端每 5s 轮询不再与写抢锁）。`:memory:` 例外（内存库每连接独立且不支持 WAL）→ 回退单连接,测试无感。连接 `Pooling=false`（单例常驻,免 Dispose 后扣文件句柄妨碍归档 VACUUM）。
