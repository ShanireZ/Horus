...
- **击键节奏**（**⚠️ 前端埋点无落点·2026-07-03 更正**）：原设计假设"判题网页前端埋点"，但**实际判题走外部洛谷(luogu.com.cn) OJ**（学员本地 IDE 写码 → 提交洛谷 → cpplearn 爬洛谷同步通过题数,见 `Cpplearn/server/luoguSync.js`）——**洛谷是外部站,无法埋点**。故击键前端节奏采集**不可实现**。**实际替代信号**：Agent 的 **OS 级剪贴板/粘贴检测**（`ClipboardWatcher` + `大段粘贴`,已实现）覆盖"粘贴外部写好的代码"这一核心信号,不依赖判题页。**服务器击键旁路 `/ingest/keystroke`(KSK 验签+归档) 保留**:若日后出现自控的本地提交页,可即接。
...
- **✅ M3 已实现（2026-07-03·owner 拍板）**：**provider-agnostic 图像嵌入器**（`IImageEmbedder`）；**规模小(单场几千图)→ C# 暴力余弦 KNN,不依赖 sqlite-vec 原生扩展**（`vec_images` 虚表留大规模余量,embedding 存普通 `image_embeddings` BLOB 表）；**仅嵌证据图**(is_evidence·省算力·后台 `ImageEmbedService` 补扫)；`POST /api/exams/{id}/search-image` 暴力余弦 top-N + 看板灯箱「按图搜图」。
...
- **M3 取证强化**：**✅ 已实现「哈希链完整性复验」**(ingest 时 canonical 逐字节复算 + `bad_hash` 拒收 + 离线 `GET /api/exams/{id}/integrity` 审计锚点自洽 / 链连续,迁移前旧数据归 `unverifiable` 不误报) + **✅「归档作业」**(`ArchiveService`:到龄考试关键数据转 archive 库 + 清理 live + VACUUM,pending 跳过,幂等崩溃安全)。**✅ CLIP 按图搜图已实现**(provider-agnostic 嵌入器·复用视觉 baseUrl/key·C# 暴力余弦·无需 sqlite-vec·仅嵌证据图·`/api/exams/{id}/search-image` + 看板灯箱「按图搜图」·见 §8)。**击键前端埋点已撤(N/A)**:判题走外部洛谷、无本地判题页可埋点,核心"粘贴外部代码"信号由 Agent OS 级剪贴板/粘贴检测覆盖(见 §6);server 侧 KSK 旁路保留待未来自控提交页。**M3 全部收口**。
- **M4 健壮性 / 身份层**：**✅ cpplearn OIDC 取代共享 PSK**（采集面 per-user 身份闭合 §10.1 事件通道栽赃 + seq 抢占，拓扑 A Server-Broker）+ **✅ RBAC 角色映射**（弟子=考生 / 长老=监考员·OIDC 取代看板静态令牌·远端工作站经自签 https·详见 [m4-identity-oidc.md](m4-identity-oidc.md) §10）+ **✅ 考前预检**（`/api/preflight`：鉴权配置 / cpplearn 可达 / active 考试白名单覆盖）。**⏳ 待做**：both→oidc 现场灰度验收。
- **M5 采集端硬化**：**✅ 已实现**——保活 / 防挂起 / 防遮蔽 / 防降权限统一为"检测 + 上报 + 看板健康告警"(4 新健康信号 + 服务器独立赋分入队 + 座位健康度)+ **三层保活看门狗**(Windows 服务 LocalSystem / 兄弟 watchdog 互拉 / 心跳告警)。纯检测·**不做内核对抗**(owner 拍板)。平台无关检测核 + 服务端 176 测试全绿;Windows 专属(服务 / SessionLauncher / 真实 suspend·遮蔽·降权)待 owner 真机验收。详见 [m5-agent-hardening.md](m5-agent-hardening.md)。
